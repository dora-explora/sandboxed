...
fn sandbox_to_text(sandbox: &Vec<Vec<u8>>) -> String {
    let mut lines: Vec<String> = vec![String::new(); sandbox[0].len()/8];
    ...
    for x in 0..sandbox.len() {
        if sandbox[x][y] == 1 && y < (sandbox[0].len() - 1) {
            if sandbox[x][y+1] == 0 {
                ...
                sandbox[x][y] = 0;
            } else if x > 0 { // currently the pixel always falls left: this should be random
